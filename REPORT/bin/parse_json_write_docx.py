...
# parse the sierrapy json output for relevant information - subtype and DRMs
    ...
    with open(json_in) as json_file:
        ...
        for i in data:
        	sample = i['inputSequence']['header'].strip()
        	
        	# Check whether the JSON sample ID matches an
        	# 'Our/Alternative ID' in the metadata TSV
        	if (sample, "Your Sample ID") not in patientdata:
        		print(
        			f"WARNING: No matching 'Our/Alternative ID' found "
        			f"for sample '{sample}'. Skipping report generation."
        		)
        		continue
        	...
        	print(
        		...
        		f"(Your Sample ID: {patientdata[(sample, 'Your Sample ID')]})"
        	)
            ...
            kh=ht0.add_run()
            kh.add_picture('logo1.png', width=Inches(4))
            ...
            kh=ht1.add_run()
            kh.add_picture('logo2.png', width=Inches(3.6))
